...
impl ToolRegistry {
    ...
    /// Return OpenAI function-calling format schemas for enabled tools.
    ///
    /// A tool is included if its name appears in `ctx.enabled_tools`.
    ...
    /// Return the toolset name for a registered tool, if any.
    #[allow(dead_code)]
